...
const { searchQuery, addTask, updateTask, deleteTask, moveTask, tasksForStatus } = useTasks()
...
<template>
  <main class="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <AppHeader v-model:search-query="searchQuery" @create="openCreate" />
    <TaskBoard :tasks-for-status="tasksForStatus" @edit-task="openEdit" @move-task="moveTask" />
    <TaskEditorModal
      :open="isEditorOpen"
      :task="editingTask"
      @close="closeEditor"
